#!/usr/bin/env python3
"""Benchmark orientation-only vs cell+orientation multitask across rooms."""
...
ROOM_CAMPAIGNS: dict[str, list[tuple[str, CampaignSpec, str]]] = {
    "E101": [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--datasets", default="E101", help="Comma-separated rooms among E101,E102.")
    parser.add_argument("--cell-weight", type=float, default=0.05, help="Relative weight of the cell loss.")
    parser.add_argument("--seeds", default="7,17,27,37,47", help="Comma-separated seeds.")
    parser.add_argument("--output-prefix", default="orientation_multitask_rooms", help="Artifact prefix.")
...
def load_dataframe(selected_rooms: list[str]) -> tuple[pd.DataFrame, list[str]]:
    frames: list[pd.DataFrame] = []
    labels: set[str] = set()
    for room in selected_rooms:
        for campaign_name, spec, orientation_label in ROOM_CAMPAIGNS[room]:
            ...
            frames.append(df)
            labels.add(orientation_label)
    combined = pd.concat(frames, ignore_index=True)
    return combined, sorted(labels, key=orientation_label_to_degrees)
...
def train_model(
    ...
    *,
    ...
    for _ in range(max_epochs):
        ...
        _, cell_val_logits, orient_val_logits = model.forward(X_val)
        orient_val_loss, _ = cross_entropy_and_grad(orient_val_logits, y_orient_val)
        val = orient_val_loss
        if multitask and y_cell_val is not None and cell_val_logits is not None:
            cell_val_loss, _ = cross_entropy_and_grad(cell_val_logits, y_cell_val)
            val += cell_weight * cell_val_loss
...
def predict_model(model: SharedMLP, scaler: StandardScaler, X: np.ndarray) -> tuple[np.ndarray | None, np.ndarray, np.ndarray]:
...
def orientation_probs_to_angles(orient_probs: np.ndarray, orient_encoder: LabelEncoder) -> tuple[np.ndarray, np.ndarray]:
    ...
    pred_labels = np.asarray([nearest_orientation_label(angle, labels=class_labels) for angle in pred_angles], dtype=object)
...
def run_seed(
    ...
    *,
    ...
    orient_only_model, orient_only_scaler = train_model(
        X_train, y_orient_train, None, seed=seed, multitask=False, cell_weight=cell_weight
    )
    multitask_model, multitask_scaler = train_model(
        X_train, y_orient_train, y_cell_train, seed=seed, multitask=True, cell_weight=cell_weight
    )
    ...
    pred_df = test_df[["room", "campaign", "grid_cell", "orientation_label", "orientation_deg"]].copy()
    pred_df["seed"] = seed
    ...
    return SplitResult(
        seed=seed,
        orient_only_acc=float(accuracy_score(test_df["orientation_label"], orient_only_labels)),
        orient_only_f1_macro=float(f1_score(test_df["orientation_label"], orient_only_labels, average="macro")),
        orient_only_mean_ang_err=mean_angular_error_deg(true_angles, orient_only_angles),
        multitask_orient_acc=float(accuracy_score(test_df["orientation_label"], multitask_labels)),
        multitask_orient_f1_macro=float(f1_score(test_df["orientation_label"], multitask_labels, average="macro")),
...
def summarise(results: list[SplitResult], pred_df: pd.DataFrame, orientation_labels: list[str]) -> dict:
    ...
    cm = confusion_matrix(pred_df["orientation_label"], pred_df["multitask_pred_label_circular"], labels=orientation_labels)
    per_room = []
    for room, group in pred_df.groupby("room", sort=True):
        per_room.append(
            {
                "room": room,
                "samples": int(len(group)),
                "orient_only_accuracy": float(accuracy_score(group["orientation_label"], group["orient_only_pred_label_circular"])),
                "multitask_orientation_accuracy": float(accuracy_score(group["orientation_label"], group["multitask_pred_label_circular"])),
                "multitask_cell_accuracy": float(accuracy_score(group["grid_cell"], group["multitask_pred_cell"])),
                "orient_only_mean_angular_error_deg": float(np.mean([circular_distance_degrees(t, p) for t, p in zip(group["orientation_deg"], group["orient_only_pred_angle_deg"], strict=False)])),
                "multitask_mean_angular_error_deg": float(np.mean([circular_distance_degrees(t, p) for t, p in zip(group["orientation_deg"], group["multitask_pred_angle_deg"], strict=False)])),
            }
        )
    return {
        "dataset": {
...
def main() -> None:
    args = parse_args()
    selected_rooms = [token.strip().upper() for token in str(args.datasets).split(",") if token.strip()]
    invalid = sorted(set(selected_rooms) - set(ROOM_CAMPAIGNS))
    if invalid:
        raise ValueError(f"Unknown rooms: {invalid}")
    seeds = [int(token.strip()) for token in str(args.seeds).split(",") if token.strip()]
    df, orientation_labels = load_dataframe(selected_rooms)
    cell_encoder = LabelEncoder().fit(df["grid_cell"])
    orient_encoder = LabelEncoder().fit(orientation_labels)
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    print(
        f"Loaded rooms={selected_rooms} | samples={len(df)} | cells={df['grid_cell'].nunique()} | "
    ...
    summary = summarise(results, pred_df, orientation_labels)
    summary["cell_weight"] = float(args.cell_weight)
    summary["rooms"] = selected_rooms
    prefix = str(args.output_prefix).strip() or "orientation_multitask_rooms"
